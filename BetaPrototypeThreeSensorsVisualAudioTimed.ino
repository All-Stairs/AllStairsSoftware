...
Multiplexer Pin             Serial Bit (Based on most recent output scheme)
A                            7
B                            6
C                            5
D                            4 not sure
...
#define irLedPin 37         // IR Led on this pin
...
byte IRMultiplexer(int triggerPin);
...
void loop() 
{  
  stepData = IRMultiplexer (irLedPin);
...
byte IRMultiplexer(int triggerPin) {
  ...
  incoming = shiftIn(dataIn, clockIn, LSBFIRST); //SHOULD WE HAVE DATA AT THIS POINT? 
